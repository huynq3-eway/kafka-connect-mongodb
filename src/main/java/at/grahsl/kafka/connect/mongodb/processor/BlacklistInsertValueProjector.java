...
public class BlacklistInsertValueProjector extends BlacklistProjector {

    private static Logger LOGGER = LoggerFactory.getLogger(BlacklistInsertValueProjector.class);
    private Predicate<MongoDbSinkConnectorConfig> predicate;

    public BlacklistInsertValueProjector(MongoDbSinkConnectorConfig config, String collection) {
        this(config,config.getInsertValueProjectionList(collection),
                cfg -> cfg.isUsingBlacklistValueProjection(collection),collection);
    }

    public BlacklistInsertValueProjector(MongoDbSinkConnectorConfig config, Set<String> fields,
    ...
    public void process(SinkDocument doc, SinkRecord orig) {
        ...
        if (operatorHeader != null && operatorHeader.value().toString().equalsIgnoreCase("c")) {
            LOGGER.debug("(process)is insert!");
